/**
 * @file 链式操作模块
 * @description 实现百度云分享链接获取 + 小红书千帆商品发布的完整流程
 */
...
export class ChainActionExecutor {
    ...
    async executeChainAction(config: ChainActionConfig): Promise<ChainActionResult> {
        ...
        try {
            ...
            this.addLog(`百度云分享成功，链接: ${baiduResult}`);
            this.addLog(`构建商品数据: ${JSON.stringify(config)}`);

            // 第2步：使用分享链接在Agiso平台发布商品
            this.addLog('第2步：在小红书千帆平台发布商品');
            const agisoResult = await this.publishToRed(config.redProduct, baiduResult);
    ...
    /**
     * 在小红书平台发布商品
     * @param redConfig 小红书商品发布配置
     * @param baiduResult 百度云分享结果
     * @returns 发布结果
     */
    private async publishToRed(
        redConfig: ShangPinData,
        baiduResult: ShareResult
    ): Promise<{ success: boolean; message: string }> {
        this.addLog('准备在小红书平台发布商品');

        try {
            // 使用动态发布能力，直接发送发布消息到background script
            this.addLog('发送千帆商品发布请求...');

            const result = await this.sendAgisoPublishRequest({
                shareUrl: baiduResult.shareUrl || '',
                shareText: baiduResult.shareText || '',
                ...redConfig
            });

            this.addLog('千帆商品发布成功');
            return result;
        } catch (error) {
            throw new Error(`千帆发布失败: ${typeof error === 'object' && error && 'message' in error && typeof (error as { message: unknown }).message === 'string' ? (error as { message: string }).message : String(error)}`);
        }
    }
     ...
     * @returns 发布结果
     */
    private async sendAgisoPublishRequest(productData: ShangPinData): Promise<{ success: boolean; message: string }> {
        return new Promise((resolve, reject) => {
            ...
            // 构建商品数据
            const shangpinData = { ...productData };
            this.addLog(`构建商品数据: ${JSON.stringify(shangpinData)}`);
