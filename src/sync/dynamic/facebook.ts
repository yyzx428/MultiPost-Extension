...
export async function DynamicFacebook(data: SyncData) {
  ...
  try {
    ...
    // 查找创建帖子按钮
    const createPostButton = document.querySelector('div[aria-label="创建帖子"]') ||
    ...
    const photoButton = Array.from(spans).find(
      span => span.textContent?.includes("照片/视频") ||
    ...
    // 查找并填写帖子内容
    const editors = document.querySelectorAll('div[contenteditable="true"][role="textbox"][spellcheck="true"][tabindex="0"][data-lexical-editor="true"]');
    ...
    if (images && images.length > 0) {
      const fileInput = document.querySelector('input[type="file"][accept="image/*,image/heif,image/heic,video/*,video/mp4,video/x-m4v,video/x-matroska,.mkv"]') as HTMLInputElement;
      if (!fileInput) {
    ...
    const postButton = document.querySelector('div[aria-label="发帖"]') ||
